...
public final class DecodeTables {

    public static final int[] TABLE_0 = {
    ...
    public static final byte[] ENCODE = new byte[256];
    public static final byte[] DECODE = new byte[256];

    static {
        for (int i = 0; i < 256; i++) {
            int v = TABLE_0[i] ^ 8;
            ENCODE[i] = (byte) v;
            DECODE[v & 0xFF] = (byte) i;
        }
    }

    // Java doesn't support unsigned types. The typical solution is to go to the next biggest type and mask
    public static int readShort(InputStream is) throws IOException {
        byte[] b = is.readNBytes(2);
        byte[] d = { DECODE[b[0] & 0xFF], DECODE[b[1] & 0xFF] };
        short s = ByteBuffer.wrap(d).order(ByteOrder.LITTLE_ENDIAN).getShort();
        return s & 0xffff;
    }
    ...
    public static int readInt(InputStream is) throws IOException {
        ...
        byte[] d = new byte[]{
                DECODE[b[0] & 0xFF], DECODE[b[1] & 0xFF],
                DECODE[b[2] & 0xFF], DECODE[b[3] & 0xFF]
        };
    ...
    public static double readDouble(InputStream is) throws IOException {
        ...
        byte[] d = new byte[8];
        for (int i = 0; i < 8; i++) d[i] = DECODE[b[i] & 0xFF];
        return ByteBuffer.wrap(d).order(ByteOrder.LITTLE_ENDIAN).getDouble();
    }

    public static int readByte(InputStream is) throws IOException {
        int c = is.read();
        return DECODE[c & 0xFF] & 0xFF;
    }

    public static byte[] readBytes(InputStream is, int n) throws IOException {
        byte[] b = is.readNBytes(n);
        byte[] out = new byte[n];
        for (int i = 0; i < n; i++) out[i] = DECODE[b[i] & 0xFF];
        return out;
    }
}
